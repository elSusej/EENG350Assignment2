...
//need to altercode to include interrupts for both motors
...
float c1; //control
float c2;
int Kp1 = 10; //change after testing
int Kp2 = 10;
int m1Pwm = 0; //set to 0 originally? 
int m2Pwm = 0;
...
void setup() {
  ...
  digitalWrite(m2VoltageSign, HIGH); //go back and change wiring for these motors so that vecelocity is positive w/both of these HIGH
...
long MyEnc(int motorSelect) { //int motorSelect
...
void loop() {
  ...
  angularVelocity1 = deltaDist1 / deltaTime;
  ...
  //NOTE: below should be modified based on whether data for motor 1 or 2 is desired (print time, voltage, and angular velocity for specified motor)
  if (current_time <= 3) { 
    ...
    if (current_time >= 0 && current_time <= 1) {
      Serial.print(m1Pwm); 
        ...
        m1Pwm = 128;
        Serial.print((batteryVoltage * m1Pwm) / 255);
      }
    Serial.print("\t");
